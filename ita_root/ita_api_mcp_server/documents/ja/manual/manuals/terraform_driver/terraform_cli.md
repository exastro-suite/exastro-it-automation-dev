...
Terraform や Terraform driverの概要および、Terraform Cloud/EP driver と共通の機能については「」を参照してください。
# コンソールメニュー構成
...
-*******************
1. Terraform CLI-->インターフェース情報 では、作業実行の情報をメンテナンス（閲覧/更新）することができます。
...
「無効」の場合、パラメータシートに値が入っている場合のみ代入値管理への登録が行われます。
           ...
           - ー
- 状態監視周期(単位ミリ秒)
           - | 「」で表示されるログのリフレッシュ間隔を入力します。
           - 〇
           - 手動入力
           - 最小値1000ミリ秒
- 進行状態表示行数
           - | 「」での進行ログ・エラーログの最大表示行数を入力します。
           - 〇
           - 手動入力
           - ー
- 備考
...
Workspaces管理
-*************
1. Terraform CLI-->Workspace管理 では、Terraformで利用するWorkspaceについてのメンテナンス（閲覧/登録/更新／/廃止リソース削除）を行います。
同一のWorkspaceを対象とした作業実行を行う場合、Terraformが生成するstateファイルはWorkspace単位で管理され、冪等性が保たれます。
-*項目**                          | **説明**                     | **入力\   | **入力方法** | **制約事項**    |
ごとに構成・管理された\      |           |              |                 |
Movement一覧
-***********
1. Terraform CLI-->Movement一覧 では、Movement名についてのメンテナンス（閲覧/登録/更新/廃止）を行います。
MovementはTerraform利用情報としてWorkspaceと紐付ける必要があるため、先に「」にて対象を登録しておく必要があります。
Movement名            | Mov\      | ○         | 手動入力  | 最大長\   |
にて登録\ |           |           |           |
-***********
1. Terraform CLI-->Module素材集 ではユーザーが作成したModuleのメンテナンス（閲覧/登録/更新/廃止）を行います。
...
Movement-Module紐付
-******************
1. Terraform CLI-->Movement-Module紐付 では、「」にて登録したMovementと「」にて登録したModule素材の紐付けについてメンテナンス（閲覧/登録/更新/廃止）を行います。
...
変数ネスト管理
-*************
1. Terraform CLI-->変数ネスト管理 では、Module素材集で登録したtfファイルで定義されている変数のタイプがlist,setかつ、その変数の中でlist,set,tuple,objectが定義されている場合、メンバー変数の最大繰返数を閲覧及び更新できます。
...
最大繰返数の上限値は「管理コンソール - 」より識別ID「MAXIMUM_ITERATION_TERRAFORM-CLI」の設定値にて、1～1024の範囲内で変更することが可能です。
...
代入値自動登録設定
-*****************
1. Terraform CLI-->代入値自動登録設定 では、パラメータシート作成機能で作成したパラメータシート（オペレーションあり）と、Movementの変数を紐付けます。
...
Movement名                        | 「」で登録した\ |           |              |                 |
Movementが表示されます。     |           |              |                 |
IaC変数(To)     | Movement名:変数\| 「terraform_cli\       | ※2        | リスト選択   | ー              |
...
**バンドルが有効なパラメータシートの場合**
バンドルが有効なパラメータシートの項目とMovementの変数を紐付ける場合、 Terraform CLI-->代入値自動登録設定 でパラメータシート(From) の代入順序を入力する必要があります。
...
ステータスが想定外エラーで終了した場合、「」の登録不備や、その他のWebコンテンツの登録不備が原因であれば、エラーログにメッセージが表示されます。
...
作業管理
-*******
1. Terraform CLI-->作業管理 では作業の履歴を閲覧できます。
-*項目**                          | **説明**                                                                  |
...
代入値管理
-*********
1. Terraform CLI-->代入値管理 では、オペレーションに紐付くMovementで利用されるModule素材の変数に代入する具体値を閲覧できます。
-*項目**                          | **説明**                                                                  |
