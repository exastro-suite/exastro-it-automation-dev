...
def main(work_dir_path, ws_db):
    ...
    # FLAG=0にすべきメニューIDのリスト（エクスポート対象かつPRIVILEGE='2'のメニュー）
    # List of MENU_IDs that should have FLAG=0 (exportable menus with PRIVILEGE='2': read-only even for ws-admin role)
    flag_0_menu_ids = [
        # workspace (3)
        '10203', '10204', '10205',
        # ansible (7)
        '20107', '20204', '20306', '20401', '20405', '20413', '20414',
        # menu_create (8)
    ...
    g.applogger.info(f"[Trace] Step 2: Set EXPORT_PERMISSION_CHECK_FLG = 0 for {len(flag_0_menu_ids)} exportable menus with PRIVILEGE='2' (read-only)")

    # ステップ2: エクスポート対象かつPRIVILEGE='2'のメニューのEXPORT_PERMISSION_CHECK_FLGを0に設定
    # Step 2: Set EXPORT_PERMISSION_CHECK_FLG = 0 for exportable menus with PRIVILEGE='2' (read-only even for ws-admin)
    if flag_0_menu_ids:
        for table in [table_name, table_name_jnl]:
            # IN句用のプレースホルダを作成
