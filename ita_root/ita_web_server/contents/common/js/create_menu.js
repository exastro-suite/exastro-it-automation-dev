...
async setup() {
    ...
    this.stringMaxByte = 65536;
    this.linkMaxByte = 8192;
...
addItemIdCheck( type, number ) {
    ...
    while ( this.menuMap.has( id ) ) number++;
    return number;
}
/*
##################################################
        ...
        for ( const entry of entries ) {
            ...
            if ( entry.isIntersecting ) {
                if ( type === 'c') {
                    ...
                    el.appendChild( _this.setColumnContents( id ) );
                } else {
                    el.querySelector('.menu-column-group-header').appendChild( _this.setGroupContents( id ) );
                }
            } else {
                if ( type === 'c') {
    ...
    const previewIntersectionObserver = this.rafThrottle( ( entries ) => {
        for ( const entry of entries ) {
            ...
            if ( entry.isIntersecting ) {
                if ( type === 'c') {
                    el.style.width = 'auto';
                    el.appendChild( _this.setPreviewColumnContents( id ) );
                } else {
...
// カラムコンテナ
previewColumnContainerHtml() {
    return `<div class="previewColumnContainer" style="min-width: 80px"></div>`;
}
// empty
previewEmptyHtml() {
