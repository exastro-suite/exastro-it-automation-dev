...
def backyard_main(objdbca, menu_create_row):
    ...
    # 処理対象のID取得
    menu_create_id = str(menu_create_row.get('MENU_CREATE_ID'))
    create_type = str(menu_create_row.get('CREATE_TYPE'))
    ...
    try:
        ...
        if sheet_type == "1":  # パラメータシート(ホスト/オペレーションあり)
            ...
            if vertical_flag:
                ...
                sql_file_path = "./libs/menu_create/sql/parameter_sheet_cmdb_vertical.sql"
                ...
                # パラメータシート用テーブル作成SQL
                sql_file_path = "./libs/menu_create/sql/parameter_sheet_cmdb.sql"
            ...
            # データシート用テーブル作成SQL
            sql_file_path = "./libs/menu_create/sql/data_sheet_cmdb.sql"
...
def _insert_t_comn_menu(objdbca, sheet_type, record_t_menu_define, menu_group_col_name, record_t_menu_column, menu_create_id):
    ...
    try:
        ...
        data_list = {
            ...
            "SORT_KEY": sort_key,
            "DISUSE_FLAG": "0",
            "LAST_UPDATE_USER": const.MENU_CREATE_USER_ID
        }
