...
from libs.ansible_pioneer_vars_listup.backyard_main import backyard_main

# import job_config as config

class AnsiblePioneerVarsListupExecutor(BaseJobExecutor):
    ...
    def queue_query_stmt(cls, conn: DBConnectCommon) -> str:
        ...
        for schema in schemata:
            query_stmt += (
                union_stmt +
                "SELECT ORGANIZATION_ID, WORKSPACE_ID, JOB_KEY, LAST_UPDATE_TIMESTAMP, 'ansible_pioneer_vars_listup' AS JOB_NAME" +
                f" FROM `{schema}`.V_COMN_PROC_LOADED_LIST" +
                f" WHERE JOB_KEY = {const.PROC_LOADED_ID_ANSIBLE_PIONEER}"
            )
    ...
    def clean_up(cls):
        ...
        """
        try:
            g.applogger.info("ansible pioneer vars listup clean_up")
            for i in range(1, 10 + 1):
                g.applogger.debug(f"ansible pioneer vars listup clean_up : {i}")
                time.sleep(1)
...
CREATE or REPLACE VIEW V_COMN_PROC_LOADED_ANSIBLE_PIONEER_VARS_LISTUP AS
...
WHERE PROC_NAME = 'ky_pioneer_varsautolistup-workflow' AND LOADED_FLG = '0';

GRANT SELECT ON TABLE V_COMN_PROC_LOADED_ANSIBLE_PIONEER_VARS_LISTUP TO ITA_USER;
