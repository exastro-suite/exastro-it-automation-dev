...
from libs.ansible_legacy_role_vars_listup.backyard_main import backyard_main

# import job_config as config

class AnsibleLegacyRoleVarsListupExecutor(BaseJobExecutor):
    ...
    def queue_query_stmt(cls, conn: DBConnectCommon) -> str:
        ...
        for schema in schemata:
            query_stmt += (
                union_stmt +
                "SELECT ORGANIZATION_ID, WORKSPACE_ID, JOB_KEY, LAST_UPDATE_TIMESTAMP, 'ansible_legacy_role_vars_listup' AS JOB_NAME" +
                f" FROM `{schema}`.V_COMN_PROC_LOADED_LIST" +
                f" WHERE JOB_KEY = {const.PROC_LOADED_ID_ANSIBLE_LEGACY_ROLE}"
            )
...
CREATE or REPLACE VIEW V_COMN_PROC_LOADED_ANSIBLE_LEGACY_ROLE_VARS_LISTUP AS
...
WHERE PROC_NAME = 'ky_legacy_role_varsautolistup-workflow' AND LOADED_FLG = '0';

GRANT SELECT ON TABLE V_COMN_PROC_LOADED_ANSIBLE_LEGACY_ROLE_VARS_LISTUP TO ITA_USER;
