...
def main():
    ...
    with open(os.environ.get('TESTDATA_CREATEUSER_SQL'), mode="w") as f,\
            closing(__connect_admin()) as conn,\
            conn.cursor() as cursor:

        #
        # Database user generate script output
        #
        cursor.execute(f"SELECT * FROM mysql.user WHERE user like 'ITA_%' or user in ('{os.environ.get('DB_USER')}')")
    ...
    with closing(__connect_admin()) as conn,\
        ...
        #
        cursor.execute(f"SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE 'ita_%' OR SCHEMA_NAME IN ('{os.environ.get('DB_DATABASE')}')")
...
def __connect_admin() -> pymysql.connections.Connection:
