...
def retry_makedirs(dir_path, raise_error=True):
    ...
    """
    g.applogger.debug(f"sa rretry_makedirs({dir_path})")
    try:
        os.makedirs(dir_path, exist_ok=True)
        return True
    except Exception as e:
        g.applogger.info("sa rretry_makedirs failed. dir_path={}".format(dir_path))
        t = traceback.format_exc()
...
def retry_copy2(src_path, dest_path, raise_error=True):
    """
        ...
        Args:
            ...
            raise_error: リトライを実施してもエラーが発生した際に例外スローするか (True: 例外スロー&ログ出力/ False: ログ出力のみ)
    """
    g.applogger.debug(f"sa rretry_copy2({src_path, dest_path})")
    try:
        ...
        g.applogger.info("sa rretry_copy2 failed. src_path={}, dest_path={}".format(src_path, dest_path))
        t = traceback.format_exc()
        g.applogger.debug(str(t))
        if raise_error is True:
            raise e
...
def retry_rmdir(dir_path, raise_error=True):
    """
        ...
        Args:
            ...
            raise_error: リトライを実施してもエラーが発生した際に例外スローするか (True: 例外スロー&ログ出力/ False: ログ出力のみ)
    """
    g.applogger.debug(f"sa rretry_rmdir({dir_path})")
    try:
        os.listdir(os.path.dirname(dir_path.rstrip('/')))  # NFSストレージ対策：属性キャッシュ更新を試みる
        os.rmdir(dir_path)
        return True
    except Exception as e:
        g.applogger.info("sa rretry_rmdir failed. dir_path={}".format(dir_path))
        t = traceback.format_exc()
        g.applogger.debug(str(t))
        if raise_error is True:
            raise e
...
def retry_remove(file_path, raise_error=True):
    ...
    try:
        ...
        g.applogger.info("sa rretry_remove failed. file_path={}".format(file_path))
        t = traceback.format_exc()
        g.applogger.debug(str(t))
        if raise_error is True:
            raise e
